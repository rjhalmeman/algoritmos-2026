...
function procure() {
    ...
    if (codigo) { // se digitou um Codigo
        aluno = procurePorChavePrimaria(codigo);
...
function salvar() {
    ...
    if (codigo && nome && altura && data_nascimento) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                aluno = new Aluno(codigo, nome, altura, data_nascimento);
                listaAluno.push(aluno);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                alunoAlterado = new Aluno(codigo, nome, altura, data_nascimento);
...
function mostrarDadosAluno(aluno) {
    document.getElementById("inputCodigo").value = aluno.codigo;
    document.getElementById("inputNome").value = aluno.nome;
    document.getElementById("inputAltura").value = aluno.altura;
    document.getElementById("inputData_nascimento").value = aluno.data_nascimento;
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
    for (let i = 0; i < listaAluno.length; i++) {
        const linha = listaAluno[i]; //variavel linha contem as informações de cada aluno
...
// Função para processar o arquivo CSV e transferir os dados para a listaAluno
        ...
        for (let i = 0; i < linhas.length; i++) {
            ...
            if (linha) { //verifica se a linha não está vazia
                ...
                if (dados.length === 4) { //verifica os seis campos
                    // Adiciona os dados à listaAluno como um objeto
                    listaAluno.push({
                        codigo: dados[0],
                        nome: dados[1],
                        altura: dados[2],
